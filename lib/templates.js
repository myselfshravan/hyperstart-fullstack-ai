...
export const createAppComponent = (
  projectPath,
  projectName,
  isPWA,
  firebaseServices,
  ...
  ${
    ...
    <div
      ${
        useShadcnUI
          ? 'className="min-h-screen flex flex-col justify-center items-center bg-background text-foreground text-center p-8"'
          : `style={{
        display: "flex",
    ...
    >
      ...
      <Card className="w-full max-w-lg">
        <CardHeader>
          ...
          <CardDescription className="text-lg">
            Your ${
              isPWA ? "PWA is" : "project is"
            } ready with modern tools and frameworks!
          </CardDescription>
        </CardHeader>
      ...
      <p style={{ fontSize: "1.1rem", color: "#555", marginBottom: "2rem" }}>
        Your ${
          isPWA ? "PWA is" : "project is"
        } ready. Start building amazing things!
      </p>
...
export const createPWAReadme = (
  ...
  const actualFramework = usingShadcnUI
    ? "Tailwind CSS + shadcn/ui"
    : cssFramework;

  const readmeContent = `# ⚡ ${projectName}

A modern React application built with Hyperstart${
    isPWA ? " with Progressive Web App (PWA) capabilities" : ""
  }.
...
${
  firebaseServices.includes("auth")
    ? "- 🔐 **Firebase Authentication** - User management and auth flows"
    : ""
}
${
  firebaseServices.includes("firestore")
...
${shadcnComponents
  .map(
    (comp) =>
      `- ✨ **${comp.charAt(0).toUpperCase() + comp.slice(1)}** component`
  )
  .join("\n")}`
    : ""
}
...
### 📦 Additional Packages
${packages
  .map((pkg) => `- **${pkg}** - ${getPackageDescription(pkg)}`)
  .join("\n")}`
    : ""
}
...
${
  packages.includes("axios")
    ? `│   │   └── axiosInstance.js # Axios configuration
`
    : ""
}${
    firebaseServices?.includes("firestore")
      ? `│   │   └── firestoreUtils.js # Firestore utilities
`
      : ""
  }${
    firebaseServices?.includes("storage")
...
### Firebase Hosting${
    firebaseServices && firebaseServices.length > 0 ? " (Recommended)" : ""
  }
\`\`\`bash
npm run build
...
${
  template === "dashboard"
    ? `- Pre-built dashboard layout with sidebar navigation
...
`
    ...
    template !== "basic"
      ? `Modify the ${template} components in \`src/pages/\` and \`src/components/${template}/\``
      : "Start building your components in `src/components/`"
  }
2. **Add Routes**: ${
    template !== "basic"
      ? `Extend the routing in \`src/main.jsx\` for additional pages`
      : "Set up routing in `src/main.jsx`"
  }
${
  firebaseServices && firebaseServices.length > 0
    ? `3. **Configure Firebase**: Update your Firebase settings and customize security rules
4. **Add Authentication**: ${
        firebaseServices.includes("auth")
          ? "Customize the auth flows and user management"
          : "Enable Firebase Auth and implement user flows"
      }
5. **Database Design**: ${
        firebaseServices.includes("firestore")
          ? "Design your Firestore collections and implement CRUD operations"
          : "Set up your data structure"
      }`
    : `3. **State Management**: Implement Redux/Zustand if needed for complex state
4. **API Integration**: ${
        packages.includes("axios")
          ? "Configure your API endpoints and implement data fetching"
          : "Add API integration for dynamic data"
      }`
}
${
  usingShadcnUI
    ? `6. **UI Enhancement**: Add more shadcn/ui components and customize the design system`
    : `6. **Styling**: Enhance the UI with additional ${actualFramework} components`
}
...
${
  actualFramework.includes("Tailwind")
...
Feel free to check the [issues page](https://github.com/myselfshravan/hyperstart-fullstack-ai/issues).
...
**Built with ⚡ [Hyperstart](https://github.com/myselfshravan/hyperstart-fullstack-ai) - The AI-first fullstack starter kit**`;
