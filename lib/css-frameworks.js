...
export const setupTailwind = (projectPath, isShadcnUI = false) => {
    if (isShadcnUI) {
        // For shadcn/ui, install additional dependencies
        run(`npm install tailwindcss @tailwindcss/vite tailwindcss-animate`, projectPath);
    } else {
    ...
    if (isShadcnUI) {
        // shadcn/ui will handle the CSS configuration
        return;
    }
...
export const setupCSSFramework = (cssFramework, projectPath, options = {}) => {
    const { isShadcnUI = false } = options;
    
    const frameworkMap = {
        "Tailwind": () => setupTailwind(projectPath, isShadcnUI),
